...
- (void)start {
    [[Connection connectionQueue] run:^{
        if (self->_socket == nil) {
            self->_socket = [[GCDAsyncSocket alloc] initWithDelegate:self delegateQueue:[[Connection connectionQueue] nativeQueue]];
            [self->_socket setIPv4Enabled:true];
            [self->_socket setIPv6Enabled:true];
//            [_socket setPreferIPv4OverIPv6:true];

            NSLog(@"Connecting to %@:%d", Connection.ip, (int) self->_port);

            __autoreleasing NSError *error = nil;
            if (![self->_socket connectToHost:Connection.ip onPort:self->_port withTimeout:12 error:&error] || error != nil) {
                if (error != nil) {
                    if ([self->_delegate respondsToSelector:@selector(onConnectionError:error:)])
                        [self->_delegate onConnectionError:self error:error];
                }
...
- (void)close {
    [[Connection connectionQueue] run:^{
        if (!self->_closed) {
            ...
            id <ConnectionDelegate> delegate = self->_delegate;
...
- (void)sendData:(NSMutableData *)data {
    [[Connection connectionQueue] run:^{
        if (!self->_closed) {
            if (self->_socket != nil) {
                [self->_socket writeData:data withTimeout:-1 tag:0];
            } else {
